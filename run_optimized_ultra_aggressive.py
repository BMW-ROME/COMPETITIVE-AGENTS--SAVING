...
class OptimizedUltraAggressiveTradingSystem:
        ...
        try:
            ...
            for symbol, position_data in positions.items():
                ...
                if qty > 0:  # Long position
                    ...
                    # Check trailing stop
                    elif (self.risk_settings['enable_trailing'] and 
                          monitor['trailing_stop_price'] > 0 and 
                          current_price <= monitor['trailing_stop_price']):
                        risk_actions.append({
                ...
                # Log position status
                self.logger.debug(f"Position Monitor {symbol}: Price=${current_price:.2f}, P&L={pnl_pct:.2%}, Trailing=${monitor['trailing_stop_price']:.2f}")
    ...
    async def detect_market_regime(self, market_data: Dict[str, Dict[str, Any]]) -> str:
        ...
        try:
            ...
            self.logger.debug(f"Market Regime: {regime}, Volatility: {avg_volatility:.4f}, Momentum: {momentum_score:.4f}")
            return regime
    ...
    def update_performance_memory(self, agent_id: str, trade_result: Dict[str, Any]):
        ...
        try:
            ...
            regime_trades = [t for t in self.performance_memory['recent_trades'] 
            ...
            if regime_trades:
                ...
                agent_performance = [(aid, wr) for aid, wr in self.performance_memory['win_rate_by_regime'][regime].items()]
    ...
    def _generate_ultra_aggressive_decision(self, agent_id: str, market_data: Dict[str, Any], 
        ...
        if agent['last_trade_time']:
            ...
            if time_since_last_trade < agent['cooldown_seconds']:
                self.logger.debug(f"Agent {agent_id} in cooldown ({time_since_last_trade:.1f}s / {agent['cooldown_seconds']}s)")
                return None

        # ULTRA AGGRESSIVE: 99% chance to make a decision
        trade_roll = random.random()
        if trade_roll > agent['trade_probability']:
            self.logger.debug(f"Agent {agent_id} failed trade probability roll ({trade_roll:.3f} > {agent['trade_probability']})")
            return None
        ...
        volatility_multiplier = max(0.5, min(2.0, base_volatility / max(volatility, 0.01)))  # Inverse volatility scaling
        ...
        self.logger.debug(f"Agent {agent_id} volatility: {volatility:.4f}, multiplier: {volatility_multiplier:.2f}, max_position: {agent_max_position:.4f}")
        ...
        if current_position > 0:
            ...
            if buying_power > 1.0:  # Only need $1 minimum to attempt trades
                ...
                if random.random() < buy_probability:
                    ...
                    target_position_value = min(max_position_value, buying_power * 0.88)  # Don't exceed 88% of buying power
        ...
        if action == 'BUY':
            ...
            if required_capital > max_capital:
                ...
                if buying_power > 1.0:
                    ...
                    self.logger.debug(f"Agent {agent_id} position scaled down: {required_capital:.2f} -> {quantity * price:.2f}")
                else:
                    return None
        ...
        self.logger.info(f"🎯 DYNAMIC DECISION - Agent {agent_id}: {action} {quantity} {symbol} @ ${price:.2f} (${position_value:.2f} required, ${buying_power:.2f} available, {volatility_pct:.2f}% volatility)")
        ...
        try:
            ...
            if action == 'BUY':
                ...
                if required_capital > buying_power * 0.99:  # Ultra aggressive: use up to 99% of buying power
                    self.logger.warning(f"Insufficient buying power for {symbol}: {required_capital:.2f} > {buying_power * 0.99:.2f}")
                    return False
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    self.logger.info(f"✅ ULTRA AGGRESSIVE: Executed {action} {quantity} shares of {symbol} at ${trade['price']:.2f}")
    ...
    async def run_cycle(self):
        ...
        try:
            ...
            for risk_action in risk_actions:
                try:
                    ...
                    if success:
                        ...
                        self.logger.info(f"🛡️ RISK MANAGEMENT: {risk_action['reason']} - Sold {risk_action['quantity']} {risk_action['symbol']}")
            ...
            self.logger.info(f"🧠 Market Regime: {current_regime} (Momentum: {self.market_regime['momentum_score']:.3f})")
            ...
            for agent_id in self.agents:
                try:
                    ...
                    if decision:
                        ...
                        self.logger.debug(f"🎯 Adaptive Decision: {agent_id} (weight: {agent_weight:.2f}) -> {decision['action']} {decision['quantity']} {decision['symbol']}")
            ...
            for agent_id, decision in agent_decisions.items():
                try:
                    ...
                    if trade_success:
                        ...
                        self.logger.info(f"✅ ADAPTIVE TRADE: {agent_id} executed {decision['action']} {decision['quantity']} {decision['symbol']} (regime: {current_regime}, weight: {decision.get('adaptive_weight', 1.0):.2f})")
                    else:
            ...
            for agent_id, perf in top_agents:
                if perf['trades'] > 0:
                    agent_weight = adaptive_weights.get(agent_id, 1.0)
                    self.logger.info(f"  🏆 Top Agent {agent_id}: {perf['trades']} trades, {perf['success_rate']:.2f} success rate, ${perf['total_pnl']:.2f} PnL (weight: {agent_weight:.2f})")
            ...
            if len(self.market_regime['regime_history']) > 5:
                ...
                self.logger.info(f"  📈 Regime Stability: {regime_stability:.2f} (recent: {' -> '.join(recent_regimes[-3:])})")
